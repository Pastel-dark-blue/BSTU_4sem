...
void CPyramid::Draw(CDC& dc, CMatrix& PView, CRect& RW)
{
	...
	CMatrix MW = SpaceToWindow(RectView, RW);					// Матрица пересчета из ВСК в ОСК
	// Готовим массив оконных координат для рисования 
	CPoint MasVert[6];											// Массив оконных координат вершин
	CMatrix V(3);
	V(2) = 1;
	// Цикл по количеству вершин - вычисляем оконные координаты вершин из видовых
	for (int i = 0; i < 6; i++)
	{
		V(0) = ViewVert(0, i); // x
		V(1) = ViewVert(1, i); // y
...
{
	// PView - координаты положения камеры
	// ViewCart - сферические координаты PView точки в декартовых
	// MV - матрица пересчета точки из мировой системы координат в видовую
	// ViewVert - координаты вершин пирамиды в видовой СК
	// MW - Матрица пересчета в ОСК
	// MasVert - Массив оконных координат вершин
	CMatrix ViewCart = SphereToCart(PView); // декартовы координаты камеры
	...
	CPen* pOldPen = dc.SelectObject(&Pen);
	// заливка
	CBrush Brush(RGB(0, 0, 255)); // Для нижнего основания
	...
	for (int i = 0; i < 3; i++) // рисуем ребра без невидимых
	{
		...
		R1 = Vertices.GetCol(i, 0, 2);	// ребро 1
		R2 = Vertices.GetCol(k, 0, 2);	// ребро 2
		CMatrix V1 = R2 - R1;
		CMatrix V2 = VE - R1;
		VN = VectorMult(V2, V1);	// вектор внешней нормали к грани (= векторному произведению двух её ребер)
		// видимость грани определяем нахождением косинуса угла между вектором внешней нормали и вектором камеры. 
		// Для этого нужно произведение векторов камеры и внешней нормали разделить на произведение их модулей (скалярное произведение)
		sm = ScalarMult(VN, ViewCart);
		if (sm >= 0)	// если скалярное произведение больше или равно нулю => грань видимая и мы её отрисовываем
		{
			dc.MoveTo(MasVert[i]);
			dc.LineTo(MasVert[k]);
	...
	{
		CBrush* topBrush = new CBrush((COLORREF)0xffffff);
