...
// Index=1 -  Зеркальная модель отражения света :[cos(a)]^n
{
	...
	CMatrix VSphere(3), // данные точки сферы (радиус, долгота, широта)
		VSphereNorm(3), // вектор нормали к поверности сферы
		PV(4);			// Координаты точки в ВСК	
	COLORREF Col;												
	double df = 0.9; // Шаг по азимуту
	double dq = 0.6; // Шаг по широте
	double kLight; // Интенсивность освещения 			
...
//------ Проходим по точкам сферы  ---------------------------------------
	for (double fi = 0; fi <= 360.0; fi += df)  // Азимут (идем по долготе сферы ("вертикальное сечение"))
		for (double q = 0; q <= 180.0; q += dq) // идем по широте ("горизонтальное сечение")
		{
