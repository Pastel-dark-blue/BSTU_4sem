...
void CPyramid::ColorDraw(CDC &dc, CMatrix& PView, CRect& RW, COLORREF color)
{
	...
	CMatrix ViewVert = MV * Vertices; // вершины МСК->ВСК 
	CRectD RectView;
	GetRect(ViewVert, RectView); //прям-к, охват проекцию пирамиды на XY ВСК
	
	CMatrix MW = SpaceToWindow(RectView, RW); // м-ца МСК->ОСК (оконная система координат)
	CPoint MasVert[6];
	CMatrix V(3);
	V(2) = 1;
	for (int i = 0; i < 6; i++)	///верш в ОСК
	{
		V(0) = ViewVert(0, i); ///x
		V(1) = ViewVert(1, i); ///y
		V = MW * V; // матр перевода * матр вершин пирамиды
		MasVert[i].x = (int)V(0);
		MasVert[i].y = (int)V(1);
	}
	...
	for (int i = 0; i < 3; i++)
	{
		...
		CMatrix V2 = VE - R1;          // Вектор – ребро к вершине
		VN = VectorMult(V2, V1);	   // вектор внешней нормали
		//Видимость каждой из грани определяем нахождением косинуса угла между вектором внешней нормали и вектором камеры.
		...
		sm = cosv1v2(VN, ViewCart);	   // cos (между нормалью к грани и наблюдателем)
